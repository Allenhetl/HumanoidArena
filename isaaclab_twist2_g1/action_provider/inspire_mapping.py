"""Inspire (因时) dexterous-hand action mapping, ported from dex-retarget.

Freeze a 6-DoF hardware action space (``a_hw_6``) for the Inspire FTP / RH56DFTP-class
hands and expand/collapse between hardware and the 12-DoF simulation joint space
(``q_sim_12`` = 6 drive joints + 6 mimic joints).

All lookups are name-based. Do not rely on anonymous array ordering from third-party
optimizers without remapping through these helpers.

Mimic coupling (unified to the official retarget URDF):
    finger intermediate = proximal * 1.0
    thumb intermediate   = thumb pitch * 1.6
    thumb distal         = thumb pitch * 2.4
"""
...
# Mimic coefficients from the retarget URDF (unified: thumb intermediate = 1.6).
COUPLING = {
