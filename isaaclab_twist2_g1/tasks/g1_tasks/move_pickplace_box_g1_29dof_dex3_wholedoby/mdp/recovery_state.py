...
def capture_recovery_state(
    ...
    *,
    ...
    scene_state_keys = tuple(scene_state) if isinstance(scene_state, MappingABC) else ()
...
def _missing_snapshot_payloads(snapshot: RecoveryStateSnapshot, env: Any) -> set[str]:
    missing: set[str] = set()
    if not isinstance(snapshot.scene_state, MappingABC) or not snapshot.scene_state:
    ...
    if snapshot.capabilities.available.get("task_counters", False):
        if not isinstance(snapshot.task_counters, MappingABC):
    ...
    if snapshot.capabilities.available.get("task_state", False):
        if snapshot.task_state is None or (
            isinstance(snapshot.task_state, MappingABC) and not snapshot.task_state
        ):
