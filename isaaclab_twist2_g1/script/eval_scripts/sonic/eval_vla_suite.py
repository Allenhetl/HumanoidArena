...
def _build_summary(results: list[dict]) -> dict:
    ...
    for row in results:
        ...
        seed_stats = stats["per_seed"].setdefault(seed_key, {"episodes": 0, "successes": 0})
        seed_stats["episodes"] += 1
        seed_stats["successes"] += int(bool(row.get("success")))
...
def _write_summary(run_dir: Path, results: list[dict]) -> None:
    ...
    summary_json_path.write_text(json.dumps(_build_summary(results), ensure_ascii=True, indent=2) + "\n")
