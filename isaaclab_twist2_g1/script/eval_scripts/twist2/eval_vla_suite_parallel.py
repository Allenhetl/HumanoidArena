...
def _partition_jobs(all_jobs: list[dict], num_workers: int) -> list[dict]:
    ...
    for model_path, jobs in jobs_by_model.items():
        proportional_workers = max(1, math.ceil(num_workers * len(jobs) / total_jobs))
...
def main() -> int:
    ...
    parser.add_argument("--env_config_yaml", type=str, default="tasks/common_env_config/twist2_default.yaml")
    ...
    task_specs = _partition_jobs(all_jobs, args.num_workers)
    print(
        f"[eval_vla_suite_parallel] total_jobs={len(all_jobs)} "
        f"num_workers={args.num_workers} task_chunks={len(task_specs)} "
