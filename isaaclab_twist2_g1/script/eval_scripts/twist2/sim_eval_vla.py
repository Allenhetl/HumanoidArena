...
def _ensure_unique_multi_image_shm_name(args) -> str:
    ...
    print(f"[sim_eval_vla] shm_name={shm_name}")
    return shm_name
...
def _build_fall_detector(env, args_cli):
    if getattr(args_cli, "disable_fall_detection", False):
        print("[sim_eval_vla] fall detection disabled")
        return None
    ...
    try:
        ...
        while simulation_app.is_running() and controller.is_running:
            ...
            if final_reward >= 1.0:
                ...
                post_termination_steps_remaining = int(spec["post_termination_record_steps"])
                if post_termination_steps_remaining <= 0:
                    break
                continue
...
def main() -> int:
    ...
    try:
        ...
        try:
            print("[sim_eval_vla] startup checkpoint=before_gym_make")
            env = gym.make(args_cli.task, cfg=env_cfg).unwrapped
            print("[sim_eval_vla] startup checkpoint=after_gym_make")
            _initialize_task_scene(env, env_cfg, args_cli, apply_optional_runtime_augments)
            print("[sim_eval_vla] startup checkpoint=after_initialize_task_scene")
            _reset_environment_for_episode(env, env_cfg, int(first_spec["episode_seed"]))
            print("[sim_eval_vla] startup checkpoint=after_initial_reset")
            ...
            print(
                f"[sim_eval_vla] task={args_cli.task} control_hz={control_hz} episodes={len(episode_specs)} lerobot_server={args_cli.lerobot_server_url}"
            )

            print("[sim_eval_vla] startup checkpoint=before_create_action_provider")
            action_provider = create_action_provider(env, args_cli)
            print("[sim_eval_vla] startup checkpoint=after_create_action_provider")
            if action_provider is None:
            ...
            print("[sim_eval_vla] startup checkpoint=after_controller_setup")

            for spec in episode_specs:
                rss_before_mb = _get_process_rss_mb()
                print(
                    f"[sim_eval_vla] episode_start seed={spec['seed']} repeat_idx={spec['repeat_idx']} "
                    f"episode_seed={spec['episode_seed']} rss_mb={rss_before_mb:.1f}"
                )
                try:
