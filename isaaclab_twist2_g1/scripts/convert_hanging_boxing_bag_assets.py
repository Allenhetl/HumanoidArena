...
def replace_bag_mesh_in_articulation(articulation_usd: str, bag_mesh_usd: str, bag_link_name: str = "bag"):
    """在 bag link 下新增 visual override，引用預旋轉 bag mesh。"""
    ...
    # 關閉舊的 visual 子樹，保留 collision 子樹以維持物理交互
    deactivated = 0
    ...
    override_path = bag_prim.GetPath().AppendChild("bag_visual_override")
    override_xform = UsdGeom.Xform.Define(stage, override_path).GetPrim()
    override_xform.GetReferences().AddReference(bag_mesh_abs)
    print(f"  [INFO] Added bag visual override: {override_path} -> {bag_mesh_abs}")
    print(f"  [INFO] Deactivated visual prims: {deactivated}")
    stage.Save()
