...
DEFAULT_OUTPUT_ROOT = Path(
    "/home/dreams/Users/taowen/HumanoidArena/lerobot/outputs/HumanoidArena_datasets/HOI_grap_cup/twist2_localdelta_v2"
)
DEFAULT_REPO_ID = "local/twist2_localdelta_v2"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Convert TWIST2 IsaacLab recordings into LeRobot local-delta v2 data. "
            "The action target is a 40D canonical input at Isaac control time: "
            "root-local xy delta, root z, root rot6d, 29 canonical joints, hand binary."
        )
    )
    parser.add_argument("--input-root", type=Path, default=DEFAULT_INPUT_ROOT)
    parser.add_argument("--output-root", type=Path, default=DEFAULT_OUTPUT_ROOT)
    parser.add_argument("--repo-id", type=str, default=DEFAULT_REPO_ID)
    parser.add_argument("--robot-type", type=str, default="unitree_g1_localdelta_v2")
    ...
    try:
        for episode_index, npz_path in enumerate(npz_paths, start=1):
            with np.load(npz_path, allow_pickle=True) as data:
                ...
                action = build_twist2_localdelta_v2_actions(
                    data,
                    num_frames=qpos.shape[0],
