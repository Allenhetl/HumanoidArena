...
VLA_STATE_DIM = VLA_LOCAL_DELTA_V2_STATE_DIM
VLA_ACTION_DIM = VLA_LOCAL_DELTA_V2_ACTION_DIM
SCHEMA_VERSION = LOCAL_DELTA_V2_SCHEMA_VERSION
...
def build_sonic_localdelta_v2_actions(
    data: np.lib.npyio.NpzFile,
    *,
    num_frames: int,
) -> np.ndarray:
    if "human_body_pos" in data:
        ...
        raise KeyError("SONIC local-delta v2 requires human_body_pos or robot_root_position")
    ...
    if "human_body_quat_w" in data:
        ...
        raise KeyError("SONIC local-delta v2 requires human_body_quat_w or robot_root_orientation")

    if "human_joint_pos" in data:
        joint_targets = np.asarray(data["human_joint_pos"], dtype=np.float32)
    elif "vla_action_joint_pos_29" in data:
        joint_targets = np.asarray(data["vla_action_joint_pos_29"], dtype=np.float32)
    elif "vla_action_raw" in data:
        action_raw = np.asarray(data["vla_action_raw"], dtype=np.float32)
        if action_raw.shape == (num_frames, VLA_ACTION_DIM):
            joint_targets = action_raw[:, 9:38]
        else:
            raise ValueError(f"Unexpected vla_action_raw shape: {action_raw.shape}")
    else:
        raise KeyError("SONIC local-delta v2 requires human_joint_pos, vla_action_joint_pos_29, or vla_action_raw")
    ...
    for i in range(num_frames):
        if i == 0:
            ...
            world_delta = (body_pos[i, :2] - body_pos[i - 1, :2]).astype(np.float32)
            root_local_xy_delta = rotate_world_delta_to_root_local(body_quat[i], world_delta)
        actions[i] = build_vla_local_delta_action(
...
def build_twist2_localdelta_v2_actions(
    data: np.lib.npyio.NpzFile,
    *,
    num_frames: int,
    control_dt: float,
    ...
    for i in range(num_frames):
        ...
        actions[i] = build_vla_local_delta_action(
            root_local_xy_delta=xy_vel_local * float(control_dt),
            root_z=root_z,
