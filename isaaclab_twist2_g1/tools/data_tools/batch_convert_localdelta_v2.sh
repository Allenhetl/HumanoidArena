...
DEFAULT_OUTPUT_ROOT="${REPO_ROOT}/lerobot/outputs/HumanoidArena_datasets_v2"
...
Batch convert first-level HOI_*/HSI_* IsaacLab recordings into local-delta v2 LeRobot datasets.
...
SONIC_CONVERTER="${REPO_ROOT}/isaaclab_twist2_g1/tools/data_tools/sonic2lerobot_localdelta_v2.py"
TWIST2_CONVERTER="${REPO_ROOT}/isaaclab_twist2_g1/tools/data_tools/twist2lerobot_localdelta_v2.py"
...
convert_backend() {
    ...
    dataset_name="${backend_dir}_localdelta_v2"
...
if [[ "${SKIP_CONVERT}" -eq 0 ]]; then
    ...
    for task_root in "${OUTPUT_ROOT}"/HOI_* "${OUTPUT_ROOT}"/HSI_*; do
        ...
        TOUCHED_TASKS["$(basename "${task_root}")"]=1
    done
fi
