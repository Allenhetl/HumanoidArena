...
def apply_grass_pbr_to_ground(
    ...
    def _collect_meshes(p):
        meshes = []
        if p.IsA(UsdGeom.Mesh):
            meshes.append(p)
        for c in p.GetChildren():
            meshes.extend(_collect_meshes(c))
        return meshes

    target_meshes = _collect_meshes(prim)
    if not target_meshes:
        target_meshes = [prim]  # 若無 mesh 子節點，綁到 prim 本身
    ...
    if prim_path == "/World/GroundPlane":
        ...
        if default_prim and default_prim.IsValid():
            for mesh_prim in _collect_meshes(default_prim):
                binding_api = UsdShade.MaterialBindingAPI.Apply(mesh_prim)
                binding_api.Bind(mtl)
                bound_count += 1

    print(f"[grass_ground_material] ✅ 草坪 PBR 材質已套用到 {bound_count} 個 mesh")
    return True
